...
services/stickers.py           категория давления → набор стикеров из stickers.json
...
После выбора самочувствия бот отправляет случайный стикер (`services/stickers.py`). Наборы лежат
в `stickers.json` — обычный список `file_id` по ключам, файл перечитывается на лету по `mtime`,
перезапуск бота ради нового стикера не нужен.

Ключ набора выбирается по цепочке **категория давления → тон → `any`**; первый непустой набор
выигрывает, отсутствие стикера — штатная ситуация (просто ничего не отправляется). Категории —
ESC/ESH 2023, решает худший из САД/ДАД (`optimal`, `normal`, `high_normal`, `grade1`, `grade2`,
`grade3`, `hypotension`); тона — `positive` / `neutral` / `negative`. Плохое самочувствие (1–2)
сдвигает тон на ступень вниз; хорошее ничего не поднимает — при плохом давлении весёлый стикер
неуместен. Последний отправленный id хранится в `user_data["last_sticker"]`, чтобы не повторяться
подряд.

**Важно**: классификация — внутренняя, только для выбора картинки. Названия категорий
пользователю не показываются: бот не трактует значения (см. дисклеймер выше). Ошибка отправки
стикера логируется и глотается — она не должна ронять запись замера.

### Напоминания

Времён может быть несколько (лимит `MAX_REMINDERS_PER_USER`, по умолчанию 5) — например 09:00 и
22:00; в одном сообщении можно ввести сразу несколько («09:00 22:00», «9:00 и 22:00»), разбор —
`_parse_times` в `handlers/settings.py`.
